...
export function connectForNative(
	mapStateToProps,
	mapDispatchToProps,
	mergeProps,
	options = {}
	  ...
	  options.skipUpdatesCondition = (props, context) => {
		  if (
			...
			props &&
			props.route &&
			props.route.name !== routeStack[routeStack.length - 1].name
		) {
			  return false
		}
		  if (
			...
			context.currentRoute.name !== routeStack[routeStack.length - 1].name
		) {
			  return false
		}
		  if (
			...
			context &&
			context.currentRoute &&
			context.currentRoute.lessonId !==
				routeStack[routeStack.length - 1].lessonId
		) {
			  return false
		}
		// Skip updates if tab is not inside an active route, unless, forced ny props
		  if (
			context.isInsideInactiveRoute &&
			context.isInsideInactiveRoute(context.store, props)
		) {
			  return false
		}
	...
	// Helps track hot reloading.
	  ...
	  return function wrapWithConnect(WrappedComponent) {
		  ...
		  class Connect extends Component {
			...
			handleChange() {
				  const skipUpdates =
					options.skipUpdatesCondition &&
					!options.skipUpdatesCondition.call(this, this.props, this.context)
		  ...
		  Connect.contextTypes = {
			  ...
			  isInsideInactiveRoute: PropTypes.func
		}
		  Connect.propTypes = {
			  store: storeShape
		}
